...
Our prescribed source language is C++ and our target architecture is x86. The project goal is to profile a set of LLVM compiled C++ benchmarks programs using the \textbf{pass} feature of the \code{opt} LLVM module.
...
\item{\textbf{Physical Implementation Description: }Details on key LLVM concepts and api's used to implement the functionality \textit{algorithm} as an \code{opt} pass.}
\item{\textbf{Benchmark Analysis: }Conjecture based running the \code{opt} pass.}
